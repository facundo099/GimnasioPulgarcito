...
CREATE TABLE [Clientes] (
  ...
  CONSTRAINT CHK_Clientes_Baja CHECK ((Estado = 1 AND FechaBaja IS NULL) OR (Estado = 0 AND FechaBaja IS NOT NULL))
);
...
CREATE TABLE [Empleados] (
  ...
  CONSTRAINT CHK_Empleados_Baja CHECK ((Estado = 1 AND FechaBaja IS NULL) OR (Estado = 0 AND FechaBaja IS NOT NULL))
);
...
CREATE TABLE [Membresias] (
  ...
  CONSTRAINT CHK_Membresia_Duracion CHECK ([DuracionDias] > 0)
);
...
CREATE TABLE [MembresiasClientes] (
  ...
  [Estado] bit NOT NULL DEFAULT 1,
  CONSTRAINT FK_MemClientes_Cliente FOREIGN KEY ([IdCliente]) REFERENCES [Clientes]([IdCliente]),
  CONSTRAINT FK_MemClientes_Membresia FOREIGN KEY ([IdMembresia]) REFERENCES [Membresias]([IdMembresia]),
  CONSTRAINT CHK_MemCliente_Fechas CHECK ([FechaFin] >= [FechaInicio])
);
